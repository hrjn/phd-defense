...
%		\end{equation*}
	...
	\uncover<4->{
	$\Rightarrow$ construction par simulation d'estimateurs, exemple du MMSE\footnote{\textit{\textbf{M}inimum \textbf{M}ean \textbf{S}quare \textbf{E}stimator}}:
		$$ \mathbb{E}_{\pi}[\VecTheta] = \int \VecTheta p(\VecTheta | \VecObs) d\VecTheta $$
	}
...
%	Plusieurs méthodes permettent d'échantillonner depuis la loi cible:
	...
	\uncover<2-4>{
		...
		\begin{itemize}
			\item en milieu urbain: [Keats et al., 2007], [Chow et al., 2008]
	...
	\uncover<3-4>{
		...
		\begin{itemize}
			...
			\item états corrélés $\Rightarrow$ {\color{lightred} non-parallélisable}
		...
		\item<4-> Algorithmes \textbf{d'échantillonnage d'importance} (\textbf{IS}\footnote{\textit{\textbf{I}mportance \textbf{S}ampling}}): tirage d'une population d'échantillons pondérés (ou particules) à partir d'une loi de proposition.
	\end{itemize}
...
\begin{frame}
	...
	\begin{figure}
			\centering
			\includegraphics[width=0.7\textwidth]{is_vanilla}
		\end{figure}
{ 		Avantages:
		\begin{itemize}
