		...
		\begin{itemize}
			\only<2>{
				...
				\begin{itemize}
					...
					\item {\color{lightred} interprétation des rétro-concentrations
					\item  requiert un modèle dual fiable}
				\end{itemize}
				\begin{figure}
					\centering
			...
			\only<3>{
				...
				\begin{itemize}
					...
					\item le {\color{lightred} choix du maillage} conditionne:
					\begin{itemize}
						\item les temps de calcul
...
{\small 
	...
	\uncover<3-4>{
		...
		\item<4-> \textbf{Echantillonnage d'Importance, ou \textit{Importance Sampling}} (IS): tirage d'une population d'échantillons pondérés (ou \textbf{particules}) à partir d'une loi de proposition.
	\end{itemize}
}
...
\uncover<3->{
	\begin{blueblock}{D-kernel PMC [Douc et al., 2007]}
		\begin{itemize}
			...
			$$ KL(\pi || \varphi_{\bm{\alpha}}) = \int \log \bigg(\dfrac{\pi(\VecTheta)}{\varphi_{\bm{\alpha}}(\VecTheta)}\bigg)\pi(\VecTheta)d\VecTheta$$
...
\begin{frame}
	...
	\end{figure}
	\only<2>{$\Rightarrow$ suppression des calculs de dispersion dans l'algorithme d'estimation}
...
% ====================================================================
% SLIDES SUPPORT QUESTIONS 
% ====================================================================

% \begin{frame}
% 	\frametitle{AMIS: temps de calcul}
% 	\begin{itemize}
% 		\item FFT 07 (modèle à bouffées gaussiennes):
% 		\begin{itemize}
% 			\item AMIS, $(K, N_p) = (10, 100)$ sans CP: $ t_{CPU} \simeq$ 18 min \\
% 			{\color{lightred}89\%} du calcul ($\simeq$  16.1min) sur le modèle de dispersion
% 		\end{itemize}
		
% 		\item Beaune (PMSS):
% 		\begin{itemize}
% 			\item 1 calcul ``direct" (SPRAY) pour une particule statistique: 30 s\\
% 			$\Rightarrow$ pour $(K, N_p) = (10, 100)$, $t_{CPU} \simeq $ {\color{lightred}8.3 h}
% 			\item temps de calcul pour 1 itération AMIS:\\
% 			\begin{tabular}{l|c}
% 				Architecture & $t_{CPU}$ \\ 
% 				\hline
% 				\textit{laptop}, HDD 5400 tr/min & {\color{lightred}6240 s} \\ 
% 				\textit{laptop}, SSD  & {\color{lightgreen}36 s} \\ 
% 				\textit{cluster} AIRAIN (stockage distribué + cache) & {\color{lightgreen} 15 s} \\ 
% 				& 
% 			\end{tabular} 
% 		\end{itemize}
% 	\end{itemize}
